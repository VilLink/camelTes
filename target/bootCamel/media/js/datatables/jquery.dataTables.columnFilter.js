...
(function($) {
	...
	function fnCreateRangeInput() {
		...
		var from = $('<input type="text" class="number_range_filter" id="'
				+ sFromId + '" rel="' + i + '"/>');
		th.append(from);
		th.append(_fnRangeLabelPart(1));
		var sToId = sTableId + 'range_to_' + i;
		var to = $('<input type="text" class="number_range_filter" id="'
				+ sToId + '" rel="' + i + '"/>');
		th.append(to);
	...
	function fnCreateDateRangeInput() {
		...
		th.html(_fnRangeLabelPart(0));
		var sFromId = sTableId + 'range_from_' + i;
		var from = $('<input type="text" class="date_range_filter" id="'
				+ sFromId + '" rel="' + i + '"/>');
		from.datepicker();
		th.append(from);
		th.append(_fnRangeLabelPart(1));
		var sToId = sTableId + 'range_to_' + i;
		var to = $('<input type="text" class="date_range_filter" id="' + sToId
				+ '" rel="' + i + '"/>');
		th.append(to);
		th.append(_fnRangeLabelPart(2));
		...
		$.fn.dataTableExt.afnFiltering.push(function(oSettings, aData,
			...
			var dCellDate = $.datepicker.parseDate(
					$.datepicker.regional[""].dateFormat, aData[index]);
	...
	function fnCreateSelect(aData) {
		var index = i;
		var r = '<select class="search_init select_filter"><option value="" class="search_init">'
				+ label + '</option>', j, iLen = aData.length;
	...
	function _fnRangeLabelPart(iPlace) {
		...
		case 1:
			return sRangeFormat.substring(sRangeFormat.indexOf("{from}") + 6,
	...
	$.fn.columnFilter = function(options) {
		...
		return this.each(function() {
			...
			if (oTable.fnSettings().oFeatures.bServerSide) {
				...
				oTable.fnSettings().fnServerData = function(sSource, aoData,
						fnCallback) {
